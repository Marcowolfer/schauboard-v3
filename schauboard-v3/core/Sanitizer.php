...
function schauboard_sanitize_block(array $block): array
{
    ...
    if (!in_array($type, ['text', 'clock', 'image'], true)) {
        $type = 'text';
    }
    ...
    return [
        ...
        'type' => $type,
        'x' => max(0, min(1720, (int) ($block['x'] ?? 120))),
        'y' => max(0, min(980, (int) ($block['y'] ?? 120))),
        'w' => max(120, min(1920, (int) ($block['w'] ?? 420))),
        'h' => max(60, min(1080, (int) ($block['h'] ?? 140))),
        'text' => schauboard_sanitize_text($block['text'] ?? ''),
        'src' => schauboard_sanitize_urlish($block['src'] ?? ''),
        'font_size' => max(16, min(160, (int) ($block['font_size'] ?? 42))),
